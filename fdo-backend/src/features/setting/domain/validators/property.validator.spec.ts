...
describe('PropertyValidator', () => {
  describe('validate', () => {
    ...
    it('should return an error for an invalid property key', () => {
      const validator = new PropertyValidator(
    ...
    it('should return an error for a non-string property value', () => {
      const validator = new PropertyValidator(PropertyKeyEnum.PREFIX, 123);
      expect(validator.validate()).toBeInstanceOf(Array<ValidationError>);
    });

    it('should return an error for a non-number property value for counter key', () => {
      const validator = new PropertyValidator(
        PropertyKeyEnum.COUNTER,
        'invalid value',
      );
